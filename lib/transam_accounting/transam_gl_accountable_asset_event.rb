module TransamGlAccountableAssetEvent
  ...
  def create_depreciation_entry
    ...
    if gl_mapping.present? # check whether this app records GLAs at all
      ...
      gl_mapping.asset_account.general_ledger_account_entries.create!(event_date: event_date, description: "Rehab #{asset.asset_path}", amount: changed_amount)

      if self.general_ledger_account.present?
        self.general_ledger_account.general_ledger_account_entries.create!(event_date: event_date, description: "Rehab #{asset.asset_path}", amount: -changed_amount)
      end
    end
