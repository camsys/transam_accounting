...
class DecliningBalanceDepreciationCalculator < DepreciationCalculator
  ...
  def calculate_on_date(asset,on_date)
    ...
    # depreciation time
    num_months = asset.depreciation_useful_life.nil? ? asset.policy_analyzer.get_max_service_life_months : asset.depreciation_useful_life

    # Depreciation months of the asset
    depreciation_months = asset.depreciation_months(on_date, asset.depreciation_entries.depreciation_expenses.where('event_date <= ?', on_date).last.event_date)
