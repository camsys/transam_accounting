...
RSpec.describe AssetsController, :type => :controller do
  ...
  it 'POST update_depreciation' do
    ...
    post :update_depreciation, use_route: :update_depreciation_inventory_path, :asset_depreciable_proxy => {:object_key => test_asset.object_key, :depreciable => '1', :depreciation_start_date => (Date.today - 1.day).strftime('%m/%d/%Y'), :salvage_value => '1111', :expected_useful_life => '111', :expected_useful_miles => '11111'}
    test_asset.reload
    ...
    expect(test_asset.expected_useful_life).to eq(111)
    expect(test_asset.expected_useful_miles).to eq(11111)
